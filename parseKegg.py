...
#   print line
	if ( strEntry in line) :
		...
		if(entryName not in dict_drugs):
			...
			dict_drugs[entryName]['GENE'] =set()		
			dict_drugs[entryName]['DBID']=None		
			dict_drugs[entryName]['SE']=set()							
	elif (strPathway in line ):
		pathwaySplit = line.split(strPathway)[1]
		pathwayId=pathwaySplit.split('  ')[0]
		pathwayId=pathwayId.split('(')[0]
...
#		print ("ya")
		...
		if('DrugBank' in dbLinks):
			...
			string = dict_drugs[entryName]['DBID']
			stringSplit = string.split('DrugBank: ')[1]
...
#				print ('\t',':',dbLinks)
				string =dbLinks
#				print ('\t',string.split('DrugBank: ')[1])
				stringSplit = removeJunk(string.split('DrugBank: ')[1])
				print ('\t',stringSplit)
				dict_drugs[entryName]['DBID'] =removeJunk(stringSplit)
		continue
